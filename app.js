...
const DATA_SOURCES = [
  ...
  {
    name: "OpenAI Models",
    type: "AI Planning, Demand Analysis & Synthesis",
    desc: "OpenAI powers the planner, demand specialist, WTO trade evidence agent, final synthesis, and non-live scoring/forecasting layers. These are informed estimates, not real-time measurements.",
    tag: "ai",
  },
  {
    name: "Claude Models",
    type: "AI Market Constraints Analysis",
    desc: "Claude is used for the market-constraints specialist to interpret pricing pressure, trade exposure, and supply-side risk using the gathered evidence.",
    tag: "ai",
  },
];
...
function renderArimaRobustnessCard(ar) {
  ...
  const candidateCount = Number(ar.candidateCount || 0);
  const testedModels = Array.isArray(ar.testedModels) ? ar.testedModels.slice(0, 3) : [];
  ...
  return `<div class="section-card" style="margin-bottom:20px">
    ...
    ${
      candidateCount
        ? `<div class="stat-sub">Best model selected from ${esc(candidateCount)} tested ARIMA specifications using holdout RMSE first, then residual/stationarity diagnostics.</div>`
        : ""
    }
    ${
      testedModels.length
        ? `<div class="stat-sub" style="margin-top:8px">Top candidates: ${esc(
            testedModels
              .map((m) => `${m.modelOrder} (RMSE ${hasNumber(m.holdoutRMSE) ? Number(m.holdoutRMSE).toFixed(2) : "n/a"})`)
              .join(" | ")
          )}</div>`
        : ""
    }
...
function formatConfidenceLabel(value = "") {
  const v = cleanPresentationText(value)
...
function parseConfidenceDisplay(value = "") {
  const cleaned = cleanPresentationText(value);
  if (!cleaned) {
    return {
      label: "Confidence not recorded",
      detail: "",
    };
  }

  const match = cleaned.match(/^\s*(high|moderate|medium|low)\b[\s\-—:;,]*(.*)$/i);
  if (!match) {
    return {
      label: formatConfidenceLabel(cleaned),
      detail: "",
    };
  }

  const level = match[1].toLowerCase() === "medium" ? "moderate" : match[1].toLowerCase();
  const detail = cleanPresentationText(match[2] || "").replace(/\bconfidence\b\.?$/i, "").trim();
  return {
...
function renderSpecialistTraceCard(agentTrace, wtoTrade = null) {
  ...
  const market = specialists.marketSpecialist || {};
  const synthesis = specialists.synthesis || {};
  ...
  const synthesisConfidence = parseConfidenceDisplay(synthesis.confidence || "");
  ...
  return `<div class="section-card specialist-card" style="margin-bottom:20px">
    ...
    <div class="specialist-grid">
      ...
      <div class="specialist-panel">
        <div class="specialist-head">
          <div>
            ...
            ${
              demandConfidence.detail
                ? `<div class="specialist-confidence-note">${esc(demandConfidence.detail)}</div>`
                : ""
            }
          </div>
      ...
      <div class="specialist-panel">
        <div class="specialist-head">
          <div>
            <div class="specialist-title">Market Constraints Specialist</div>
            <div class="specialist-subtitle">Claude analyst</div>
            ${
              marketConfidence.detail
                ? `<div class="specialist-confidence-note">${esc(marketConfidence.detail)}</div>`
                : ""
            }
          </div>
      ...
      <div class="specialist-panel">
        <div class="specialist-head">
          <div>
            <div class="specialist-title">WTO Trade Evidence Agent</div>
            <div class="specialist-subtitle">${esc(tradeProviderLabel)}</div>
            ${
              tradeConfidence.detail
                ? `<div class="specialist-confidence-note">${esc(tradeConfidence.detail)}</div>`
                : ""
            }
          </div>
      ...
      ${
        w
          ? `<div class="specialist-section-label">Trade & Tariff Context</div>
        <div class="specialist-note">HS Category: ${esc(cleanPresentationText(w.usedHsCategory || w.hsCategory || "Not specified"))}</div>
        <div class="specialist-note">Latest tariff: ${esc(
          hasNumber(Number(w.latestTariffPercent)) ? Number(w.latestTariffPercent).toFixed(1) + "%" : "N/A"
        )}${w.latestYear ? ` in ${esc(String(w.latestYear))}` : ""}</div>
        <div class="specialist-note">Partner candidates: ${esc(
          Array.isArray(w.partnerCandidateNames) && w.partnerCandidateNames.length
            ? w.partnerCandidateNames.join(", ")
            : Array.isArray(w.partnerCandidates) && w.partnerCandidates.length
            ? w.partnerCandidates.join(", ")
            : "None"
        )}</div>
        ${
          Array.isArray(w.criticalComponents) && w.criticalComponents.length
            ? `<div class="specialist-note">Critical components: ${esc(w.criticalComponents.join(", "))}</div>`
            : ""
        }`
          : ""
      }
    ...
    <div class="specialist-synthesis">
      <div class="specialist-synthesis-head">
        <div>
          <div class="specialist-title">Final Synthesis</div>
          ${
            synthesisConfidence.detail
              ? `<div class="specialist-confidence-note">${esc(synthesisConfidence.detail)}</div>`
              : ""
          }
        </div>
      ...
      <div class="specialist-two-col">
        <div>
          ...
          <div class="specialist-note">${esc(cleanPresentationText(synthesis.agreementSummary || "No agreement summary recorded."))}</div>
        </div>
        <div>
          <div class="specialist-section-label">What needed reconciliation</div>
          <div class="specialist-note">${esc(cleanPresentationText(synthesis.tensionSummary || "No tension summary recorded."))}</div>
        </div>
      </div>
      <div class="specialist-section-label">Recommended action</div>
      <div class="specialist-action">${esc(cleanPresentationText(synthesis.recommendedAction || "No recommended action recorded."))}</div>
    </div>
  </div>`;
}
...
function renderResults() {
  ...
  const p = r.pricing || {};
  const timelineIsLive = String(r.timelineMethod || "").startsWith("google_");
  ...
  html += `<div class="data-legend">
    ...
    <div class="data-legend-item"><div class="legend-dot ai"></div>AI estimate (OpenAI + Claude)</div>
  </div>`;
  ...
  html += renderSpecialistTraceCard(r.agentTrace, w);
  ...
  if (w && !(r?.agentTrace?.specialists && typeof r.agentTrace.specialists === "object")) {
    ...
    html += `<div class="section-card" style="margin-bottom:20px">
      ...
      ${
        Array.isArray(w.criticalComponents) && w.criticalComponents.length
          ? `<div class="stat-sub" style="margin-top:4px">Critical components: ${esc(w.criticalComponents.join(", "))}</div>`
          : ""
      }
      ${
        w.productType
          ? `<div class="stat-sub" style="margin-top:4px">Trade profile: ${esc(w.productType)}</div>`
          : ""
      }
      ${
        w.warning
...
function buildReportHTML(r, n, t, w) {
  ...
  const specialists = agentTrace?.specialists && typeof agentTrace.specialists === "object" ? agentTrace.specialists : null;
  const toolPlan = Array.isArray(agentTrace?.toolPlan) ? agentTrace.toolPlan : [];
  const additionalTools = Array.isArray(agentTrace?.additionalTools) ? agentTrace.additionalTools : [];
  const toolRuns = Array.isArray(agentTrace?.toolRuns) ? agentTrace.toolRuns : [];
  ...
  const specialistTraceHTML = specialists
    ? (() => {
        ...
        const bullets = (items) => {
          const arr = Array.isArray(items) ? items.filter(Boolean).slice(0, 4) : [];
          if (!arr.length) return `<div style="font-size:10px;color:#8c897e">No notable signals recorded.</div>`;
          return arr
            .map(
              (item) =>
                `<div style="font-size:11px;color:#4a4740;line-height:1.5;margin-bottom:5px">• ${esc(cleanPresentationText(item))}</div>`
            )
            .join("");
        };
  ...
  <div class="grid-2" style="margin-bottom:10px">
    ...
    <div class="stat" style="border-color:#d9d4ff;background:#faf8ff">
      ...
      ${
        w
          ? `<div style="font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:0.07em;color:#8c897e;margin:10px 0 6px">Trade & Tariff Context</div>
      <div class="report-body" style="font-size:11px">HS Category: ${esc(cleanPresentationText(w.usedHsCategory || w.hsCategory || "Not specified"))}</div>
      <div class="report-body" style="font-size:11px">Latest tariff: ${esc(
        hasNumber(Number(w.latestTariffPercent)) ? Number(w.latestTariffPercent).toFixed(1) + "%" : "N/A"
      )}${w.latestYear ? ` in ${esc(String(w.latestYear))}` : ""}</div>
      <div class="report-body" style="font-size:11px">Partner candidates: ${esc(
        Array.isArray(w.partnerCandidateNames) && w.partnerCandidateNames.length
          ? w.partnerCandidateNames.join(", ")
          : Array.isArray(w.partnerCandidates) && w.partnerCandidates.length
          ? w.partnerCandidates.join(", ")
      ...
      ${
        Array.isArray(w.criticalComponents) && w.criticalComponents.length
          ? `<div class="report-body" style="font-size:11px">Critical components: ${esc(w.criticalComponents.join(", "))}</div>`
          : ""
      }`
          : ""
      }
    </div>
    <div class="stat" style="border-color:#cfece5;background:#fcfffd">
  ...
  <div class="stat" style="margin-bottom:10px;border-color:#ddd6fe;background:#faf7ff">
    ...
    <div class="grid-2" style="margin-top:10px">
      <div>
        ...
        <div class="report-body" style="font-size:11px">${esc(cleanPresentationText(synthesis.agreementSummary || "No agreement summary recorded."))}</div>
      </div>
      <div>
        <div style="font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:0.07em;color:#8c897e;margin-bottom:6px">What needed reconciliation</div>
        <div class="report-body" style="font-size:11px">${esc(cleanPresentationText(synthesis.tensionSummary || "No tension summary recorded."))}</div>
      </div>
    </div>
    <div style="font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:0.07em;color:#8c897e;margin:10px 0 6px">Recommended action</div>
    <div class="report-body" style="font-size:11px">${esc(cleanPresentationText(synthesis.recommendedAction || "No recommended action recorded."))}</div>
  </div>`;
      })()
    : "";
  ...
  ${
    Array.isArray(w.criticalComponents) && w.criticalComponents.length
      ? `<div style="font-size:10px;color:#8c897e;margin-top:4px">Critical components: ${esc(w.criticalComponents.join(", "))}</div>`
      : ""
  }
  ${
    w.productType
      ? `<div style="font-size:10px;color:#8c897e;margin-top:4px">Trade profile: ${esc(w.productType)}</div>`
      : ""
  }
  ${
    w.warning
...
<style>*{box-sizing:border-box;margin:0;padding:0}body{font-family:-apple-system,'Segoe UI',Helvetica,Arial,sans-serif;font-size:13px;color:#1a1814;background:#fff;line-height:1.5}.page{max-width:780px;margin:0 auto;padding:40px 40px 60px}.rpt-header{display:flex;justify-content:space-between;align-items:flex-start;padding-bottom:16px;border-bottom:3px solid #2563eb;margin-bottom:24px}.rpt-logo{font-size:18px;font-weight:800;color:#2563eb}.rpt-meta{text-align:right;font-size:11px;color:#8c897e;line-height:1.8}h1{font-size:26px;font-weight:700;letter-spacing:-0.02em;margin-bottom:4px}h1 em{color:#2563eb;font-style:italic}.sub{font-size:11px;color:#8c897e;margin-bottom:20px}.sec-title{font-size:10px;font-weight:700;letter-spacing:0.09em;text-transform:uppercase;color:#8c897e;margin:22px 0 10px;padding-top:16px;border-top:1px solid #e4e1d8}.grid-3{display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin-bottom:4px}.grid-4{display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin-bottom:4px}.grid-2{display:grid;grid-template-columns:repeat(2,1fr);gap:10px}.stat{background:#f8f7f4;border:1px solid #e4e1d8;border-radius:7px;padding:12px 14px;position:relative;overflow:hidden}.stat.live{border-color:#99f6e4}.stat.live::before{content:'';position:absolute;top:0;left:0;right:0;height:2px;background:#0d9488}.stat-lbl{font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:0.07em;color:#8c897e;margin-bottom:5px}.stat-val{font-size:22px;font-weight:800;line-height:1;margin-bottom:3px;letter-spacing:-0.02em}.stat-sub{font-size:10px;color:#8c897e}.c-blue{color:#2563eb}.c-green{color:#16a34a}.c-red{color:#dc2626}.c-purple{color:#7c3aed}.c-amber{color:#d97706}.c-teal{color:#0d9488}.badge-pill{display:inline-block;font-size:8px;font-weight:700;padding:1px 6px;border-radius:10px;margin-left:6px;vertical-align:middle}.badge-pill.live{background:#f0fdfa;color:#0d9488;border:1px solid #99f6e4}.badge-pill.ai{background:#f5f3ff;color:#7c3aed;border:1px solid #ddd6fe}.bar-row{margin-bottom:9px}.bar-lbl{display:flex;justify-content:space-between;font-size:11px;color:#4a4740;margin-bottom:4px}.bar-lbl span:last-child{font-weight:700}.bar-track{height:7px;background:#f1efe9;border-radius:4px;overflow:hidden}.bar-fill{height:100%;border-radius:4px}.tl-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin-bottom:4px}.tl-box{border:1px solid #e4e1d8;border-radius:7px;padding:14px;background:#f8f7f4}.tl-box.future{border-color:#bfcff8;background:#eff4ff}.tl-period{font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:0.07em;color:#8c897e;margin-bottom:2px}.future .tl-period{color:#2563eb}.tl-year{font-size:18px;font-weight:800;margin-bottom:7px}.badge{display:inline-block;font-size:9px;font-weight:700;padding:2px 8px;border-radius:20px;margin-bottom:7px}.badge-green{background:#f0fdf4;color:#16a34a;border:1px solid #bbf7d0}.badge-amber{background:#fffbeb;color:#d97706;border:1px solid #fde68a}.badge-red{background:#fef2f2;color:#dc2626;border:1px solid #fecaca}.badge-purple{background:#f5f3ff;color:#7c3aed;border:1px solid #ddd6fe}.badge-gray{background:#f1efe9;color:#4a4740;border:1px solid #e4e1d8}.tl-body{font-size:10.5px;color:#4a4740;line-height:1.6}.report-body{font-size:12.5px;color:#4a4740;line-height:1.75;white-space:pre-wrap}.chip-group{display:flex;flex-wrap:wrap;gap:5px;margin-bottom:5px}.chip{font-size:11px;font-weight:500;padding:4px 10px;border-radius:20px;border:1px solid #e4e1d8;color:#4a4740;background:#f1efe9}.chip.hot{background:#eff4ff;color:#2563eb;border-color:#bfcff8}.chip.warm{background:#fffbeb;color:#d97706;border-color:#fde68a}.chip.cold{background:#f1efe9;color:#8c897e;text-decoration:line-through}.chip.sub{background:#f5f3ff;color:#7c3aed;border-color:#ddd6fe}.kw-lbl{font-size:9.5px;font-weight:700;text-transform:uppercase;letter-spacing:0.06em;color:#8c897e;margin:10px 0 5px}.brand-row{display:flex;align-items:center;gap:8px;padding:8px 12px;border:1px solid #e4e1d8;border-radius:5px;margin-bottom:5px;font-size:12px;font-weight:500;background:#f8f7f4}.brand-row.lead{border-color:#bfcff8;background:#eff4ff}.brand-row.rise{border-color:#bbf7d0;background:#f0fdf4}.brand-num{width:18px;height:18px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:9px;font-weight:800;flex-shrink:0}.brand-num.lead{background:#2563eb;color:white}.brand-num.rise{background:#16a34a;color:white}.prod-box{padding:9px 11px;border:1px solid #e4e1d8;border-radius:5px;background:#f8f7f4;font-size:11px;color:#4a4740}.ps{font-size:8.5px;font-weight:700;text-transform:uppercase;letter-spacing:0.06em;margin-bottom:3px}.ps.hot{color:#16a34a}.ps.fade{color:#8c897e}.prod-box.fading{opacity:0.6}.drv-lbl{font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:0.06em;margin-bottom:8px}.drv-lbl.bull{color:#16a34a}.drv-lbl.bear{color:#dc2626}.drv{display:flex;gap:6px;padding:7px 10px;border-radius:5px;margin-bottom:5px;font-size:11px;line-height:1.5;color:#4a4740}.drv.bull{background:#f0fdf4;border:1px solid #bbf7d0}.drv.bear{background:#fef2f2;border:1px solid #fecaca}.verdict{padding:12px 16px;background:#f5f3ff;border:1px solid #ddd6fe;border-radius:7px;display:flex;gap:12px;margin-top:14px}.verdict-lbl{font-size:9px;font-weight:800;text-transform:uppercase;letter-spacing:0.07em;color:#7c3aed;white-space:nowrap}.verdict-txt{font-size:11.5px;color:#4a4740;line-height:1.65}.src-row{display:flex;gap:12px;padding:12px 0;border-bottom:1px solid #e4e1d8}.src-row:last-child{border-bottom:none}.src-num{width:20px;height:20px;border-radius:50%;background:#f1efe9;border:1px solid #e4e1d8;display:flex;align-items:center;justify-content:center;font-size:9px;font-weight:800;color:#8c897e;flex-shrink:0}.src-name{font-size:12px;font-weight:700;margin-bottom:1px}.src-type{font-size:9.5px;color:#8c897e;margin-bottom:4px;font-family:monospace}.src-desc{font-size:11px;color:#4a4740;line-height:1.5}.specialist-citations{display:flex;flex-direction:column;gap:7px;margin:6px 0 2px}.specialist-citation{display:flex;justify-content:space-between;gap:12px;align-items:center;text-decoration:none;border:1px solid #e4e1d8;border-radius:8px;background:#ffffff;padding:8px 10px;font-size:11px;color:#4a4740;line-height:1.45}.specialist-citation-host{flex-shrink:0;font-size:9px;text-transform:uppercase;letter-spacing:.05em;color:#8c897e}.disclaimer{background:#fffbeb;border:1px solid #fde68a;border-radius:7px;padding:12px 16px;font-size:11px;color:#92400e;line-height:1.6;margin-top:14px}.footer{margin-top:32px;padding-top:12px;border-top:1px solid #e4e1d8;font-size:10px;color:#8c897e;display:flex;justify-content:space-between}</style></head><body><div class="page">
  ...
  <div style="display:flex;gap:12px;margin-bottom:16px;font-size:10px;color:#4a4740;align-items:center">
    ...
    <span style="display:flex;align-items:center;gap:4px"><span style="width:8px;height:8px;border-radius:50%;background:#7c3aed;display:inline-block"></span> AI estimate (OpenAI + Claude)</span>
  </div>
  ...
  ${
    candidateCount
      ? `<div style="font-size:10px;color:#8c897e;margin-top:8px">Best model selected from ${esc(candidateCount)} tested ARIMA specifications using holdout RMSE first, then residual/stationarity diagnostics.</div>`
      : ""
  }
  ${
    testedModels.length
      ? `<div style="font-size:10px;color:#8c897e;margin-top:4px">Top candidates: ${esc(
          testedModels
            .map((m) => `${m.modelOrder} (RMSE ${hasNumber(m.holdoutRMSE) ? Number(m.holdoutRMSE).toFixed(2) : "n/a"})`)
            .join(" | ")
        )}</div>`
  ...
  <div class="disclaimer"><strong>Disclaimer:</strong> News coverage and Google Trends are fetched live (best-effort; Google Trends may be rate-limited). Pricing uses live eBay sold/completed data when available; otherwise pricing falls back to active listings or AI estimates. Non-live analysis, specialist reasoning, scores, and forecasts are AI-generated estimates using OpenAI and Claude, not real-time facts. Always verify with primary sources before making decisions.</div>
